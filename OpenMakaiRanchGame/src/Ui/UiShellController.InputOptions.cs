...
public partial class UiShellController
{
    ...
    {
        var row = new HBoxContainer { Name = $"Binding_{descriptor.Action}", SizeFlagsHorizontal = SizeFlags.ExpandFill };
        ...
        reset.Pressed += () =>
        {
            ...
            if (_controlsLiveStatus is not null) _controlsLiveStatus.Text = $"Reset {descriptor.DisplayName}.";
            _game.Feedback.PlayConfirm();
        };
        row.AddChild(reset);
        return row;
    ...
    public override void _Input(InputEvent @event)
    {
        ...
        if (_bindingCaptureDevice == "keyboard" && @event is InputEventKey key && key.Pressed && !key.Echo)
        {
            ...
            if (keycode == Key.Escape) CancelBindingCapture("Binding cancelled.");
        ...
        {
            if ((int)joyButton.ButtonIndex == 1) CancelBindingCapture("Binding cancelled.");
            else
            {
        ...
        {
            InputBindingService.SetGamepadAxisBinding(_bindingCaptureAction, (int)joyMotion.Axis, joyMotion.AxisValue);
    ...
    private void CancelBindingCapture(string message)
    {
        ...
        _bindingCaptureDevice = string.Empty;
        _bindingCaptureButton = null;
        if (_controlsLiveStatus is not null) _controlsLiveStatus.Text = message;
    }
    ...
    private void EnsureControllerFocus()
    {
        ...
        if (_navButtons.TryGetValue(_currentScreen, out var current)
            && GodotObject.IsInstanceValid(current) && current.Visible && !current.Disabled)
        {
            current.GrabFocus();
        }
    }
