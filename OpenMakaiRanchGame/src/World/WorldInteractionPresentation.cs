...
public partial class RanchGreyboxController
{
    public WorldInteractionPresentation GetInteractionPresentation()
    {
        ...
        {
            return new WorldInteractionPresentation(_travelPortal, _travelPortal!.Prompt, _travelPortalDistance, InteractionRange, true, string.Empty);
        }

        if (npcValid && (!stationValid || _nearbyCharacterDistance < _nearbyDistance))
        {
            return new WorldInteractionPresentation(npc, $"Talk to {ResolveCharacterName(_nearbyCharacterId)}", _nearbyCharacterDistance, InteractionRange, true, string.Empty);
        }

        if (stationValid)
        {
            return new WorldInteractionPresentation(_nearbyStation, _nearbyStation!.Label, _nearbyDistance, InteractionRange, _nearbyStation.IsAvailable, _nearbyStation.UnavailableReason ?? string.Empty);
        }
...
public partial class TownWorldController
{
    public WorldInteractionPresentation GetInteractionPresentation()
    {
            ...
            && (!serviceValid || _nearbyCompanionDistance < _nearbyServiceDistance))
        {
            ...
            return new WorldInteractionPresentation(companion, $"Talk to {name}", _nearbyCompanionDistance, InteractionRange, true, string.Empty);
        }

        if (portalValid && (!serviceValid || _returnPortalDistance <= _nearbyServiceDistance))
        {
            return new WorldInteractionPresentation(_returnPortal, _returnPortal!.Prompt, _returnPortalDistance, InteractionRange, true, string.Empty);
        }

        if (serviceValid)
        {
            return new WorldInteractionPresentation(_nearbyService, _nearbyService!.Label, _nearbyServiceDistance, InteractionRange, _nearbyService.IsAvailable, _nearbyService.UnavailableReason ?? string.Empty);
        }
